function [len, angle, interaction,activation] = find_interaction(scene,superclass,activation,labels,mask,bboxe,len,angle)
interaction = zeros(length(activation),2);
if(scene=="other")
    for i=1:size(angle,1)
        if(superclass(i).value=="accessory")
            overlapRatio = 0;
        ...
        if(labels(i)=="person")
            overlapRatio = 0;
            for j=1:size(angle,1)
                if(superclass(j).value=="vehicle_h" && activation(j)==1)
                    overlapRatio = bboxOverlapRatio(bboxe(i,:),bboxe(j,:));
                    if(overlapRatio>0.2)
                        interaction(i,1)=i;
                        interaction(i,2)=j;
