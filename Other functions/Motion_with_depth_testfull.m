...
%     [len, angle,interaction, activation] = find_interaction1(scene,superclass,activation,data.label,data.masks,data.bbox,len,angle);
...
%     valid_object = distortion_object_blur_depth(D,name,mask,bboxe,lens,angles,outputFolder,outputHead);
    valid_object = distortion_object_blur_depth(D,name,mask,bboxe,lens,angles,outputFolder,outputHead,mask_out,mask_old);
