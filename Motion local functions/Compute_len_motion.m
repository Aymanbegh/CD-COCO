%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Function to compute the orientation and intensity of object motions by considering the depth
...
for i=1:size(data.bbox,1)
    ...
    if(data.label(i)=="person")
        ...
        if(scene_context=="sport")
            if(scene=="indoor")
                interval_speed = [5 (person_in - ((person_in-9)*(1-ratio_angle)))];
            end       
            if(scene=="outdoor")
                if(angle(i)<45 && angle(i)>-45)
                    ratio_angle = abs(45-angle(i))/45;
                end
                interval_speed = [8 (person_out - ((person_out-13)*(1-ratio_angle)))];
            end
        else    
            if(scene=="indoor")
                interval_speed = [3 (person_in - ((person_in-5)*(1-ratio_angle)))];
            end       
            if(scene=="outdoor")
